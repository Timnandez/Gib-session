            ...
            Timnandez.ev.on("connection.update", async (s) => {
                ...
                if (connection === "open") {
                    ...
                    const sid = megaUrl.includes("https://mega.nz/file/")
                        ? 'Gibdon-MD~' + megaUrl.split("https://mega.nz/file/")[1]
                        : 'Error: Invalid URL';

                    console.log(`Session ID: ${sid}`);

                    const session = await Timnandez.sendMessage(Malvin.user.id, { text: sid });

                    const TIMNANDEZ_TEXT = `
🎉 *Welcome to Gibson-MD!* 🚀  
...
💡 *Whats Next?* 
1️⃣ Explore all the cool features of Gibson-MD.
...
🚀 _Thanks for choosing Gibson-MD — Let the automation begin!_ ✨`;

                    await Malvin.sendMessage(Timnandez.user.id, { text: TIMNANDEZ_TEXT }, { quoted: session });
